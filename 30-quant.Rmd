...
The [Imputing quantitative proteomics
data](https://rformassspectrometry.github.io/QFeatures/articles/Imputation.html)
describes the `impute()` method with many examples.
...
Below we apply PCA using the processed peptide level data and the summarized
protein level data. Since PCA does not accept missing values we first need to
remove `NA` using `filterNA`. Note that several alternative dimension
reduction methods allow the use of incomplete data (e.g. nipals [@wold:1966] or
bpca [@bishop:1998])
